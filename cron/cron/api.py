...
def maintenance_clean_up() -> Any:
    number_cleaned = repositories.event_repository.clean_items_before(datetime.now() - timedelta(hours=2))
...
def maintenance_clean_up_all() -> Any:
    number_cleaned = repositories.event_repository.clean_items_before(datetime(9999, 1, 1, 1, 1, 1))
    for venue in repositories.venue_repository.fetch_all():
        venue.last_fetched_date = datetime.min
        repositories.venue_repository.upsert(venue)
    logging.getLogger(__name__).info("Number of items cleaned %d", number_cleaned)
    return Response(status=200)
