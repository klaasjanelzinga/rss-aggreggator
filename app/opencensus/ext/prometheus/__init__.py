...
class Options(object):
    ...
    def __init__(self, namespace="", port=8000, address="", registry=CollectorRegistry()):
...
class Collector(object):
    ...
    def register_view(self, view):
        ...
        if v_name not in self.registered_views:
            desc = {"name": v_name, "documentation": view.description, "labels": list(map(sanitize, view.columns))}
    ...
    def to_metric(self, desc, tag_values, agg_data):
        ...
        if isinstance(agg_data, aggregation_data_module.CountAggregationData):
            ...
            metric.add_metric(
                labels=tag_values, buckets=buckets, sum_value=agg_data.sum,
            )
            return metric
