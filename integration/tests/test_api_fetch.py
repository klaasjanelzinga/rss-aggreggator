...
@pytest.mark.asyncio
async def test_first_fetch(client_session, api_url):
    result = await client_session.get(f"{api_url}/api/events", params={"fetch_offset": ""})
...
@pytest.mark.asyncio
async def test_paging(client_session, api_url):
    result = await client_session.get(f"{api_url}/api/events", params={"fetch_offset": ""})
    assert_that(result.status, equal_to(200))
    json = await result.json()
    assert_that(len(json["events"]), equal_to(25))
    ...
    token: str = json["fetch_offset"]
    result = await client_session.get(f"{api_url}/api/events", params={"fetch_offset": token})
    assert_that(result.status, equal_to(200))
    json = await result.json()
    assert_that(len(json["events"]), equal_to(25))
