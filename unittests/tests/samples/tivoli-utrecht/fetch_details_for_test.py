...
2. fetch detail pages using this script. generates the files in this directory.
"""
...
async def fetch(filename: str, client_session: ClientSession):
    ...
    with open(filename) as json_fp:
        ...
        for i in data:
            url = i["link"]
            url_parts = url.split("/")
            out_filename = f"{directory}/{url_parts[-2]}"
...
async def fetch_all():
    async with ClientSession(timeout=ClientTimeout(40)) as client_session:
        await asyncio.gather(
            fetch(
                "tests/samples/tivoli-utrecht/admin-ajax.php?action=get_events&page=1&categorie=&maand=", client_session
            ),
            fetch(
                "tests/samples/tivoli-utrecht/admin-ajax.php?action=get_events&page=2&categorie=&maand=", client_session
            ),
        )
